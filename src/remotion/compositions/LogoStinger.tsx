...
const LOGO_W = 576;
const LOGO_H = 192;
const PL = 1000; // normalized pathLength for all paths
...
export const LogoStinger: React.FC<Props> = () => {
  ...
  // All paths draw simultaneously via trimpath
  const strokeP = spring({ frame, fps, config: { damping: 22, stiffness: 55, mass: 1.3 } });
  const dashOffset = interpolate(strokeP, [0, 1], [PL, 0]);

  // Fill fades in as stroke finishes, stroke fades out
  const fillOpacity = interpolate(frame, [48, 78], [0, 1], { extrapolateRight: "clamp", extrapolateLeft: "clamp" });
  const strokeOpacity = interpolate(frame, [48, 82], [1, 0], { extrapolateRight: "clamp", extrapolateLeft: "clamp" });
